...
<div class="row g-4">
  <div class="col-lg-4">
    ...
    <div class="cur-ticker">
      ...
      <div class="cur-flag">{{ $country->flag_emoji??'🏳️' }}</div>
      <div class="cur-country">{{ $country->name }}</div>
      <div class="cur-pair">1 {{ $base }} = ? {{ $target }}</div>
      <div class="cur-rate">{{ $currentRate?number_format($currentRate,$currentRate<1?6:2):'—' }}</div>
      <div class="cur-rate-sub">{{ $target }} · {{ $country->currency_name }}</div>
      ...
      <div class="cur-stat-strip">
        <div class="cur-stat-row">
          <div>
            ...
            <div class="cur-stat-value">{{ $rateRecord->rate_previous?number_format($rateRecord->rate_previous,2):'—' }}</div>
          </div>
          <div>
            <div class="cur-stat-label">Perubahan</div>
            <div class="cur-trend-tag {{ $trendClass }}">{{ $trendIcon }} {{ !is_null($changeVal)?number_format(abs($changeVal),2).'%':'—' }}</div>
          </div>
        </div>
        <div class="cur-risk-bar-shell">
          <div class="cur-risk-head">
            <span>Currency Risk</span>
            <span style="color:{{ $riskColor }}">{{ number_format($riskScore,1) }}/100 · {{ $riskLabel }}</span>
          </div>
          <div class="cur-risk-track">
            <div class="cur-risk-fill" style="width:{{ min(100,$riskScore) }}%;background:{{ $riskColor }}"></div>
          </div>
        </div>
      </div>
      @endif
    ...
    <div class="cur-info-card">
      <div class="cur-info-row"><span class="cur-info-label">Negara</span><span class="cur-info-value">{{ $country->name }}</span></div>
      <div class="cur-info-row"><span class="cur-info-label">Kode</span><span class="cur-info-value">{{ $target }}</span></div>
      <div class="cur-info-row"><span class="cur-info-label">Nama</span><span class="cur-info-value">{{ $country->currency_name }}</span></div>
      <div class="cur-info-row"><span class="cur-info-label">Base</span><span class="cur-info-value">{{ $base }}</span></div>
      <a href="{{ route('countries.show',$country->code) }}" class="cur-btn">
  ...
  <div class="col-lg-8">
    <div class="cur-panel">
      <div class="cur-panel-head"><i class="bi bi-graph-up"></i>Tren Kurs 30 Hari — {{ $base }}/{{ $target }}</div>
    ...
    <div class="row g-3">
      <div class="col-6 col-md-3"><div class="cur-metric"><div class="cur-metric-label">Tertinggi</div><div class="cur-metric-value">{{ number_format($mx,2) }}</div></div></div>
      <div class="col-6 col-md-3"><div class="cur-metric"><div class="cur-metric-label">Terendah</div><div class="cur-metric-value">{{ number_format($mn,2) }}</div></div></div>
      <div class="col-6 col-md-3"><div class="cur-metric"><div class="cur-metric-label">Rata-rata</div><div class="cur-metric-value">{{ number_format($avg,2) }}</div></div></div>
      <div class="col-6 col-md-3"><div class="cur-metric">
        <div class="cur-metric-label">Perubahan 30h</div>
        <div class="cur-metric-value" style="color:{{ $chg>0?'var(--up)':($chg<0?'var(--down)':'var(--text-dim)') }}">{{ $chg>0?'+':'' }}{{ number_format($chg,2) }}%</div>
      </div></div>
    </div>
    @endif
  </div>
...
if(rt.length){
  ...
  new Chart(ctx,{
    type:'line',
    data:{labels:lb,datasets:[{label:'{{ $base }}/{{ $target }}',data:rt,borderColor:'#F0B84B',backgroundColor:grad,tension:.4,fill:true,pointRadius:0,borderWidth:2}]},
    options:{
      responsive:true,
