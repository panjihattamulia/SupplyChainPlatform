...
<div class="cmp-history mt-4">
  ...
  <table>
    <tbody>
      ...
      <tr>
        <td>{{ $snap->countryA->flag_emoji??'' }} {{ $snap->countryA->name??$snap->country_a }}</td>
        <td style="text-align:center">⚖️</td>
        <td>{{ $snap->countryB->flag_emoji??'' }} {{ $snap->countryB->name??$snap->country_b }}</td>
        <td><button class="replay-btn" onclick="replay('{{ $snap->country_a }}','{{ $snap->country_b }}')">{{ $snap->recommendation }}</button></td>
        <td class="cmp-time">{{ $snap->created_at->diffForHumans() }}</td>
      </tr>
      @endforeach
    </tbody>
  </table>
