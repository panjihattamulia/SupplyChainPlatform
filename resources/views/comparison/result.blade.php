...
<div class="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-2">
  ...
  <a href="{{ route('comparison.index') }}" class="cmp-back"><i class="bi bi-arrow-left"></i>Bandingkan Lagi</a>
</div>

<div class="row g-3 mb-3 align-items-center">
  <div class="col-md-5">
    <div class="cmp-card cmp-card--a {{ $W===$A['country_code']?'is-winner':'' }}" style="color:{{ $W===$A['country_code']?'#3FD0C9':'inherit' }}">
      <div class="cmp-flag">{{ $A['flag_emoji']??'🏳️' }}</div>
      <div class="cmp-name" style="color:var(--text)">{{ $A['country_name'] }}</div>
      <div class="cmp-score">{{ number_format($A['total_score'],1) }}</div>
      <span class="cmp-badge" style="color:var(--a-color);border-color:var(--a-color)">{{ $A['risk_label']??'' }}</span>
      @if($W===$A['country_code'])<div class="cmp-crown">🏆 Direkomendasikan</div>@endif
    </div>
  </div>
  <div class="col-md-2 text-center"><div class="cmp-vs-roundel">VS</div></div>
  <div class="col-md-5">
    <div class="cmp-card cmp-card--b {{ $W===$B['country_code']?'is-winner':'' }}" style="color:{{ $W===$B['country_code']?'#EF5B5B':'inherit' }}">
      <div class="cmp-flag">{{ $B['flag_emoji']??'🏳️' }}</div>
      <div class="cmp-name" style="color:var(--text)">{{ $B['country_name'] }}</div>
      <div class="cmp-score">{{ number_format($B['total_score'],1) }}</div>
      <span class="cmp-badge" style="color:var(--b-color);border-color:var(--b-color)">{{ $B['risk_label']??'' }}</span>
      @if($W===$B['country_code'])<div class="cmp-crown" style="color:var(--b-color)">🏆 Direkomendasikan</div>@endif
    </div>
  </div>
</div>
...
<div class="cmp-duel">
  <div class="cmp-duel-head">
    <span>{{ strtoupper($A['country_name']) }}</span>
    <span>DUEL METER</span>
    <span>{{ strtoupper($B['country_name']) }}</span>
  </div>
  <div class="cmp-duel-track">
    <div class="cmp-duel-fill-a" style="width:{{ $pctA }}%"></div>
    <div class="cmp-duel-center"></div>
    <div class="cmp-duel-fill-b" style="width:{{ $pctB }}%"></div>
  </div>
...
<div class="cmp-table-panel">
  ...
  <div class="table-responsive">
    <table class="cmp-table">
      <thead>
        <tr><th>Indikator</th><th>{{ $A['country_name'] }}</th><th>{{ $B['country_name'] }}</th><th>Lebih Baik</th></tr>
      </thead>
      <tbody>
        @foreach([
        ...
        <tr>
          <td>{{ $r['l'] }}</td>
          <td class="{{ $r['wa']?'cmp-win-a':'' }}">{{ $r['a'] }}</td>
          <td class="{{ !$r['wa']?'cmp-win-b':'' }}">{{ $r['b'] }}</td>
          <td><span class="cmp-win-tag">{{ $r['wn'] }}</span></td>
        </tr>
        @endforeach
      </tbody>
    </table>
...
<div class="cmp-verdict">
  ...
  <div class="row align-items-center g-3">
    <div class="col-auto"><div class="cmp-verdict-flag">{{ $rec['flag_emoji']??'🏳️' }}</div></div>
    <div class="col">
      <div class="cmp-verdict-name">{{ $rec['country_name'] }}</div>
      <p class="cmp-verdict-reason">{{ $comparison['recommendation_reason'] }}</p>
    </div>
    <div class="col-auto">
      <div class="cmp-verdict-score">{{ number_format($rec['total_score'],1) }}</div>
