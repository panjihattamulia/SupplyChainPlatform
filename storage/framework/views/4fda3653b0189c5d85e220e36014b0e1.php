...
<div class="cd-hero">
  <div class="row align-items-center g-3">
    <div class="col-auto"><div class="cd-hero-flag"><?php echo e($country->flag_emoji??'🏳️'); ?></div></div>
    <div class="col">
      <div class="cd-hero-eyebrow">Country Dossier</div>
      <h1 class="cd-hero-name"><?php echo e($country->name); ?></h1>
      <div class="cd-hero-meta"><?php echo e($country->capital); ?> · <?php echo e($country->region); ?> · <?php echo e($country->subregion); ?></div>
    ...
    <div class="col-auto">
      ...
      <div class="cd-hero-score" style="color:<?php echo e($rColor); ?>"><?php echo e(number_format($riskScore['total_score'],1)); ?></div>
      <span class="cd-hero-badge" style="color:<?php echo e($rColor); ?>;border-color:<?php echo e($rColor); ?>;background:rgba(255,255,255,.04)"><?php echo e($rl); ?></span>
    </div>
    <?php endif; ?>
  </div>
</div>
...
<div class="row g-4">
  ...
  <div class="col-lg-8">
    ...
    <div class="cd-panel">
      <div class="cd-panel-head">
        ...
        <small>Weather(<?php echo e($riskScore['weather_weight']); ?>%) + Inflation(<?php echo e($riskScore['inflation_weight']); ?>%) + Currency(<?php echo e($riskScore['currency_weight']); ?>%) + News(<?php echo e($riskScore['news_weight']); ?>%)</small>
      </div>
      <div class="cd-panel-body">
        <div class="cd-risk-grid">
          ...
          <div class="cd-risk-item">
            <i class="bi <?php echo e($c['icon']); ?>" style="color:<?php echo e($c['color']); ?>"></i>
            <div class="cd-risk-value" style="color:<?php echo e($c['color']); ?>"><?php echo e(number_format($c['score'],1)); ?></div>
            <div class="cd-risk-label"><?php echo e($c['label']); ?></div>
          </div>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <div class="d-flex justify-content-between small mb-1">
          <span class="text-muted" style="color:var(--text-dim)!important">Total</span>
          <span class="fw-semibold" style="font-family:'IBM Plex Mono',monospace"><?php echo e(number_format($riskScore['total_score'],1)); ?>/100</span>
        </div>
        <div class="cd-progress-track">
          <div class="cd-progress-fill" style="width:<?php echo e($riskScore['total_score']); ?>%;background:<?php echo e($rColor); ?>"></div>
        </div>
    ...
    <div class="cd-panel">
      ...
      <div class="cd-panel-body">
        ...
        <div class="cd-econ-grid">
          <?php if(!empty($economic['gdp'])): ?><div class="cd-econ-item"><div class="cd-econ-label">GDP</div><div class="cd-econ-value"><?php $g=$economic['gdp'];echo $g>=1e12?'$'.round($g/1e12,2).'T':($g>=1e9?'$'.round($g/1e9,2).'B':'$'.number_format($g));?></div></div><?php endif; ?>
          <?php if(!empty($economic['inflation'])): ?><div class="cd-econ-item"><div class="cd-econ-label">Inflasi</div><div class="cd-econ-value" style="color:<?php echo e(abs($economic['inflation'])>10?'var(--coral)':'var(--text)'); ?>"><?php echo e(number_format($economic['inflation'],2)); ?>%</div></div><?php endif; ?>
          <?php if(!empty($economic['population'])): ?><div class="cd-econ-item"><div class="cd-econ-label">Populasi</div><div class="cd-econ-value"><?php $p=$economic['population'];echo $p>=1e9?round($p/1e9,2).'B':($p>=1e6?round($p/1e6,1).'M':number_format($p));?></div></div><?php endif; ?>
          <?php if(!empty($economic['exports'])): ?><div class="cd-econ-item"><div class="cd-econ-label">Ekspor</div><div class="cd-econ-value" style="color:var(--cyan)"><?php $e=$economic['exports'];echo $e>=1e12?'$'.round($e/1e12,2).'T':($e>=1e9?'$'.round($e/1e9,2).'B':'$'.number_format($e));?></div></div><?php endif; ?>
          <?php if(!empty($economic['imports'])): ?><div class="cd-econ-item"><div class="cd-econ-label">Impor</div><div class="cd-econ-value" style="color:var(--coral)"><?php $i=$economic['imports'];echo $i>=1e12?'$'.round($i/1e12,2).'T':($i>=1e9?'$'.round($i/1e9,2).'B':'$'.number_format($i));?></div></div><?php endif; ?>
          <?php if(!empty($economic['unemployment'])): ?><div class="cd-econ-item"><div class="cd-econ-label">Pengangguran</div><div class="cd-econ-value"><?php echo e(number_format($economic['unemployment'],2)); ?>%</div></div><?php endif; ?>
        </div>
        <?php else: ?>
  ...
  <div class="col-lg-4">
    ...
    <div class="cd-weather-card">
      ...
      <div class="cd-weather-icon"><?php echo e($wicon); ?></div>
      <div class="cd-weather-temp"><?php echo e($weather['temperature_2m']??'--'); ?>°C</div>
      <div class="cd-weather-desc"><?php echo e($weather['weather_description']??''); ?></div>
      <div class="cd-weather-strip">
        <div><div class="cd-weather-strip-label">HUJAN</div><div class="cd-weather-strip-value"><?php echo e($weather['precipitation']??0); ?>mm</div></div>
        <div><div class="cd-weather-strip-label">ANGIN</div><div class="cd-weather-strip-value"><?php echo e($weather['windspeed_10m']??0); ?>km/h</div></div>
        <div><div class="cd-weather-strip-label">LEMBAB</div><div class="cd-weather-strip-value"><?php echo e($weather['humidity']??'--'); ?>%</div></div>
      </div>
      <?php if($weather['is_storm']||$weather['is_heavy_rain']||$weather['is_strong_wind']): ?>
      <div class="cd-weather-flags">
        <?php if($weather['is_storm']): ?><span class="cd-weather-flag" style="color:var(--coral);border-color:rgba(239,91,91,.4)">⛈️ Storm</span><?php endif; ?>
        <?php if($weather['is_heavy_rain']): ?><span class="cd-weather-flag" style="color:#5EA8E0;border-color:rgba(94,168,224,.4)">🌧️ Heavy Rain</span><?php endif; ?>
        <?php if($weather['is_strong_wind']): ?><span class="cd-weather-flag" style="color:var(--amber);border-color:rgba(240,169,71,.4)">💨 Strong Wind</span><?php endif; ?>
      </div>
      <?php endif; ?>
      <div class="cd-weather-risk">Weather Risk: <b><?php echo e(number_format($weather['weather_risk_score']??0,1)); ?>/100</b></div>
    </div>
    <?php endif; ?>

    <div class="cd-panel">
      <div class="cd-panel-body d-grid gap-2">
        <button id="wlBtn" class="cd-action-btn cd-action-btn--wl <?php echo e($isWatchlisted?'is-active':''); ?>"
          onclick="<?php echo e($isWatchlisted?'removeWl':'addWl'); ?>('<?php echo e($country->code); ?>')">
          <i class="bi bi-star<?php echo e($isWatchlisted?'-fill':''); ?>"></i>
          <?php echo e($isWatchlisted?'Hapus dari Watchlist':'Tambah ke Watchlist'); ?>

        </button>
        <a href="<?php echo e(route('comparison.index')); ?>?a=<?php echo e($country->code); ?>" class="cd-action-btn"><i class="bi bi-bar-chart-steps"></i>Bandingkan Negara</a>
        <a href="<?php echo e(route('visualization.show',$country->code)); ?>" class="cd-action-btn"><i class="bi bi-graph-up"></i>Data Visualization</a>
        <a href="<?php echo e(route('weather.show',$country->code)); ?>" class="cd-action-btn"><i class="bi bi-cloud-sun"></i>Weather Detail</a>
        <a href="<?php echo e(route('ports.index')); ?>?country=<?php echo e($country->code); ?>" class="cd-action-btn"><i class="bi bi-anchor"></i>Pelabuhan</a>
        <a href="<?php echo e(route('currency.show',$country->code)); ?>" class="cd-action-btn"><i class="bi bi-currency-exchange"></i>Currency Detail</a>
      </div>
    </div>
    ...
    <div class="cd-panel mb-0">
      ...
      <div class="cd-panel-body">
        <div class="cd-info-row"><span class="cd-info-label">Kode ISO</span><span class="cd-info-value"><?php echo e($country->code); ?> / <?php echo e($country->code3); ?></span></div>
        <div class="cd-info-row"><span class="cd-info-label">Region</span><span class="cd-info-value"><?php echo e($country->region); ?></span></div>
        <div class="cd-info-row"><span class="cd-info-label">Ibu Kota</span><span class="cd-info-value"><?php echo e($country->capital); ?></span></div>
        <div class="cd-info-row"><span class="cd-info-label">Mata Uang</span><span class="cd-info-value"><?php echo e($country->currency_code); ?></span></div>
        <?php if($country->population): ?>
        <div class="cd-info-row"><span class="cd-info-label">Populasi</span><span class="cd-info-value"><?php $p=$country->population;echo $p>=1e9?round($p/1e9,2).'B':($p>=1e6?round($p/1e6,1).'M':number_format($p));?></span></div>
        <?php endif; ?>
        <?php if($country->languages): ?>
        <div class="cd-info-row"><span class="cd-info-label">Bahasa</span><span class="cd-info-value"><?php echo e(implode(', ',array_slice((array)$country->languages,0,3))); ?></span></div>
        <?php endif; ?>
      </div>
    </div>
  </div>
